...
import traceback # mod Ab

print('file: mainUIdetector_v4_Ab_4.py') # mod Ab
...
##    im88 = ctx.GevGetImageBuffer().reshape(height, width)
    ...
    for track in tracks:
        try:
            ...
            newImg = track.plot()
        # mod Ab 1-2-24 11:28:00 , intended to print try block error alongisde Error en count
        ...
        # except Exception as e:
            ...
            print("Error en count")
            traceback.print_exc() # Ab mod
            
        # except:
            # print("Error en count")
        # end mod Ab 1-2-24 11:28:00
        try:
...
def plot():
    ...
    if all(value == 0 for value in bar_container.datavalues):
    	...
    	print(f"horaPlot: {horaPlot}")
    	print(f"factor: {factor}")
    	print(f"width: {width}")
    	# ax.bar_label(bar_container, fmt='{:.0f}%', label_type='center')
    	
    	heights = [bar.get_height() for bar in bar_container]
    	# heights = [0 if h is None else h for h in heights]
    	labels = [f'{h:.0f}%' if h > 0 else '' for h in heights]
    	# ax.bar_label(bar_container, fmt=lambda x: f'{x:.0f}%' if x > 0 else '', label_type='center')
    	ax.bar_label(bar_container, labels=labels, label_type='center')
