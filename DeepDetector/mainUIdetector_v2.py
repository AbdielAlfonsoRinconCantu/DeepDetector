...
global formato, formatoId, conf, meta, lastTrackId, hora, placas, factor, promedio, modelPieza
promedio = 0
lastTrackId = 0
...
def getVideoPlaca():
    ...
    im0 = cv2.cvtColor(im0, cv2.COLOR_BAYER_BG2BGR)
    ...
    tracks = model.track(im0, stream=True, persist=True, show=False)
...
def plot():
    ...
    """
    bar_container = ax.bar(hora, factor, width)
...
def queryData():
    global hora, placas, factor
    query = "SELECT hora, count(piezasMeta) as placas, round(((piezasReal * 1.0 / piezasMeta * 1.0)*100),1) as factor " \
